...
// POST /api/transfers/free-agent   海捞（新 RC 不设限，签入费=新 RC×30%）〔增量 5 后续〕
// POST /api/transfers/activation   激活（训练营 5m / 普通倍数价）〔增量 5 后续〕
// POST /api/transfers/match        匹配（新 RC > 最高价 + 差额销毁）〔增量 5 后续〕
