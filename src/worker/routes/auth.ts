// 统一认证接入（迁移步骤②，auth 项目 PRD P0-5）：OIDC RP 四端点。
...
// → 登出先吊销本地行再跳认证中心 end_session → back-channel 按 sid 吊销。
// 不调 userinfo：过渡期 sub 即 tour user id，姓名/角色由 club 查 TOUR_DB，不信任令牌声明。
