...
| 球员详情 | `.dossier` 档案卡：左球员卡常驻（可成长/到顶胶囊、初始归属行），右侧页内页签 `.seg` 档案/属性/成长——档案=合同卷宗；属性=FC 源数据重排（位置矩阵主位深色、角色带 ++金/+蓝、星级行 ★☆、六组细分卡绿橙红三档色阶、门将追加 GKP 共七组、六维雷达静态 SVG 门将换轴 DIV/HAN/KIC/REF/POS/SPD、PlayStyles 徽章）；成长=卷宗时间线 |
| 球员库 | 全联盟公开名册：顶部 `.seg` 切「当前 / 初始」视图（初始=导入时底册：建档俱乐部、建档 CA、导入 PA）；筛选条（位置/状态/可成长 `.seg-mini`/排序+方向/姓名搜索）+ keyset 游标分页（上一页/下一页，游标栈回翻）；家族表格列 UID/姓名(链接)/归属/位置/年龄/CA/PA/成长胶囊/声望/身价/状态徽章/徽章数；不显示工资——那是合同卷宗里的事 |
