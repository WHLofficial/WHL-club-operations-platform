...
- 尾行：经纪人性格章（🕴 温和/普通/苛刻，增量 27 由「经纪人档位」改名）+ 注册状态胶囊；挂牌中的球员卡加 `.badge.sky`「挂牌中」。

**档案卡 `.dossier`** —— 球员详情页容器：

- 左：`.player-card`；右：卷宗区（标题「合同卷宗」+ 打字机风条款表格：违约金/工资/签约赛季/效力时长/保护期/合同类型/source；增量 25 起效力与保护期按转会窗刻度展示，子区块用 `border-top: 1px dashed` 分隔，家族惯例）；
...
- **emoji 白名单（功能必需，仅此三组）**：满意度 😍🙂😐😠（TECH_DESIGN §6.9 文案）；金银徽章 🥇🥈；经纪人性格 🕴（增量 27 改名，DB 列仍叫 `agent_tier`）。白名单外出现 emoji 视为 bug。
...
| 旁路 | 方式 | 「两种方式」「方式单据」 |
| 收口 | 截止处理 | 「保护期从审核通过那一刻重新起算」（增量 25：`protection_ticks` 置为当前窗数，判定即刻失效；文案见 Club.tsx 续约/解约提示行） |
...
| 页面 | 布局与体验要点 |
|---|---|
| 登录 / 建队 | 家族 `.auth-card`（380px 窄卡 + 焦橙顶边）；认证码绑定 = 「档案登记卡」文案；绑定成功盖 `.stamp-ok`。增量 7：交互与文案不变（仍是 8 位认证码一格一填），烧码真源上收认证中心；新错误文案「认证码无效或已过期」「该账号已经绑定了俱乐部，解绑需联系管理组」「认证中心暂不可用，请稍后再试」沿用现有 toast/banner 样式 |
| 球队中心 | 容器 960px（沿赛事）；顶部球队头（队徽 TeamLogo 模式 + 余额大字 mono + 工资帽余量条）；级别徽章可空（派生值，空显「未定级」灰）；注册工作台顶部报名状态条（增量 9）：未报定级赛事 `banner bad` 红「尚未在赛事平台报名，请等待赛事平台管理员确认报名」，已报名 `banner ok` 绿+级别徽章；阵容名单家族表格（一行一球员，缩略球员卡列可选）；一线队/训练营 `.seg` 分段；注册校验结果用 banner |
| 球员详情 | `.dossier` 双栏：左栏 = 球员卡 + **六维雷达卡**常驻（可成长/非成长胶囊；雷达自增量 30 起从属性页搬到球员卡下方，门将换轴 DIV/HAN/KIC/REF/POS/SPD），右侧页内页签 `.seg` **合同 / 属性 / 成长 / 转会记录**——合同=合同卷宗（两列键值表 `.dossier-table`：标签列定宽、值列统一左对齐、数值列等宽 `tabular-nums`，增量 30 修对齐与字体）；属性=FC 源数据重排（位置矩阵主位深色、角色带 ++金/+蓝、星级行 ★☆、六组细分卡绿橙红三档色阶、门将追加 GKP 共七组、**PlayStyles 卡嵌在属性网格第二行空位**（跨两列，网格桌面固定 4 列 / 900px 以下 2 列，增量 30）、PlayStyles 徽章——**按槽位渲染全 15 槽**（银 `PSID1-12` + 金 `PSID13-15`，只渲染有值的槽、不给空槽占位，增量 29）；成长=卷宗时间线（增量 14 删掉「初始归属」行；徽章墙 🥇 x/3 · 🥈 x/12，带徽章的升级方案须先选满 PlayStyle 才可确认，中国计划球员另有徽章发放块，增量 30）；转会记录=只列已完成单据（完成时间/类型/转出/转入/费用/赛季，自由身一侧显「自由身」，空态「卷宗里还没有转会记录」，增量 30） |
| 球员库 | 全联盟公开名册：顶部 `.seg` 切「当前 / 初始」视图（初始=导入时底册：建档 CA、导入 PA；归属无「初始」维度——增量 14 删掉 initial_club_id，两种视图都显示**当前**归属）；**筛选与显示列在球员列左侧的 sticky 定宽栏**（桌面 260px、可收起且收起态记本地；窄屏 900px 改左侧滑出抽屉，遮罩 / × / Esc 三条关闭路），工具条只留搜索框（占位只有「查找」二字）+ 视图段（整行 sticky 吸顶，「筛选（N）」按钮兼作抽屉入口），工具条三件与左栏两行都是**同行控件底边齐平**（`.control-row` + `--control-h` 统一高度，不再靠各自 margin 凑）；**位置 / 显示列 / PlayStyle 三个多选下拉**（原生 Popover 面板，进 top layer 不被抽屉裁切，触发器贴视口底时自动翻到上方），位置只放 12 个码位、PlayStyle 顶层分「银徽章 / 金徽章」两段（各 36 项，段内按射门/传球/防守/控球/体格/门将分组）、显示列的「恢复自动」移入面板页脚；「更多筛选」里**同一属性的上下限成对一行两列**（左最低、右最高，7 对区间 + 属性 1 对 + 合同 3 对）、属性下拉按七组小标题写中文名；生效条件摘要条**与翻页条合并成表格正上方一行**（chips 靠左、翻页条靠右），无筛选时不渲染摘要条，位置一条 chip 清整类、银/金徽章各一条；**翻页条是游标式、不显示总数**（增量 28 去整表 COUNT 的连带契约变更）：「‹ 上一页」+「第 N 页 · 已加载 N 名 · 还有更多／已到末页」+「下一页 ›」，首页/末页跳转仍在（游标栈回翻）；**排序改点表头**（29 键、两态循环升→降→升、默认态不给任何列打 active）+ keyset 游标分页（游标栈回翻）；姓名搜索**去变音**（「sesko」能搜到「Šeško」），聚焦才预载轻量名册做本地推荐、点条目跳详情；家族表格列 UID/姓名(链接)/归属/位置/年龄/CA/PA/成长胶囊/声望/身价/状态徽章/徽章数；不显示工资——那是合同卷宗里的事（增量 26 改版、增量 27 收口、增量 29 补表格与槽位口径）；**单元格一律不折行**（增量 29：列宽不够改由 `.table-wrap` 横向滚动 —— 实测表格最小宽 876px vs 桌面容器 854px，移动端必然横滚）；**多选面板高度上限由组件守**（480px / 70vh，增量 29：原先那条 CSS 上限被内联 `maxHeight` 顶掉，高窗口下面板可长到接近满屏） |
...
| 海捞自由球员（增量 14） | 转会市场页下方「海捞自由球员」表：姓名格在有名分的球员后带灰色东家徽章（如「AC米兰(CPU)」——CPU 队球员在转会意义上视同海里球员，徽章让人看清他为什么在池子里）；本窗禁签红徽章照旧；名单上限 300（池里含 4 支 CPU 队约 107 人 + 待业球员） |
...
| 审核队列（管理组） | 待审单 = 待批文件卡（单据摘要 + 证据备注）；批准/驳回按钮点击即盖章动效 + `.stamp-ok/.stamp-bad` 落卡。增量10：异常出价单卡角标 `⚠️ 异常出价 ×N`（title 列判据明细）；普通/激活成交卡带裁定价输入，按裁定价批准并注明裁定前後价 |
| 市场干预（管理组） | 审核页下方干预工具条：单据 ID 输入 + 五动作（撤出价/强制送审/强制作废/强制成交/作废签约），全部 window.prompt 要原因，成功/失败走 toast |
| 球员批量维护 | 管理端 textarea（等宽字体），`球员ID 字段=值` 每行一名；行内错误即时红显，提交分片 200 |
| 俱乐部转会禁令 | 俱乐部行「转会冻结」红徽章 + 冻结/解冻按钮；教练端俱乐部页 banner-bad 提示 |
| 主场档案（增量 12） | 教练端球队中心「主场档案」卡：球场名/档位（地区级…）/容量/死忠 mono 数、影响力构成（球员+队壳+奖励分=合计，球员项自动算）；设施等级徽章行；无球场行整卡不渲染。管理端俱乐部行「主场」按钮 → 编辑卡（球场名/容量/档位 0-4/队壳影响力/奖励分，仅此五项） |
| 赛季管理（管理组） | 建季表单带「可成长年龄上限」可选输入（placeholder 25/24/23…，15-40 校验，建档 toast 带重判人数）；赛季行「结算体检」按钮 → blockers/warnings 清单展示；「结算赛季」两段式：软警示 409 时 window.confirm 列提示 → 确认带 acknowledged 重发；赛事绑定行「完结结算」按钮 + 已完结结算灰徽章（stage_settled_at） |
