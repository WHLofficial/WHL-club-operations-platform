...
| 球员库 | 全联盟公开名册：顶部 `.seg` 切「当前 / 初始」视图（初始=导入时底册：建档 CA、导入 PA；归属无「初始」维度——增量 14 删掉 initial_club_id，两种视图都显示**当前**归属）；**筛选与显示列在球员列左侧的 sticky 定宽栏**（桌面 260px、可收起且收起态记本地；窄屏 900px 改左侧滑出抽屉，遮罩 / × / Esc 三条关闭路），工具条只留搜索框 + 视图段（整行 sticky 吸顶，「筛选（N）」按钮兼作抽屉入口），生效条件摘要条常驻、每条 chip 可单独撤销；**排序改点表头**（29 键、两态循环升→降→升、默认态不给任何列打 active）+ keyset 游标分页（上一页/下一页，游标栈回翻）；姓名搜索**去变音**（「sesko」能搜到「Šeško」），聚焦才预载轻量名册做本地推荐、点条目跳详情；家族表格列 UID/姓名(链接)/归属/位置/年龄/CA/PA/成长胶囊/声望/身价/状态徽章/徽章数；不显示工资——那是合同卷宗里的事（增量 26 改版） |
