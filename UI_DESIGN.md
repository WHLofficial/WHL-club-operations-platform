...
| 球员库 | 全联盟公开名册：顶部 `.seg` 切「当前 / 初始」视图（初始=导入时底册：建档 CA、导入 PA；归属无「初始」维度——增量 14 删掉 initial_club_id，两种视图都显示**当前**归属）；筛选条（位置/状态/可成长 `.seg-mini`/排序+方向/姓名搜索）+ keyset 游标分页（上一页/下一页，游标栈回翻）；家族表格列 UID/姓名(链接)/归属/位置/年龄/CA/PA/成长胶囊/声望/身价/状态徽章/徽章数；不显示工资——那是合同卷宗里的事 |
