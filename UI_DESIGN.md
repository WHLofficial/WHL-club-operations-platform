...
| 球员库 | 全联盟公开名册：顶部 `.seg` 切「当前 / 初始」视图（初始=导入时底册：建档 CA、导入 PA；归属无「初始」维度——增量 14 删掉 initial_club_id，两种视图都显示**当前**归属）；**筛选与显示列在球员列左侧的 sticky 定宽栏**（桌面 260px、可收起且收起态记本地；窄屏 900px 改左侧滑出抽屉，遮罩 / × / Esc 三条关闭路），工具条只留搜索框（占位只有「查找」二字）+ 视图段（整行 sticky 吸顶，「筛选（N）」按钮兼作抽屉入口），工具条三件与左栏两行都是**同行控件底边齐平**（`.control-row` + `--control-h` 统一高度，不再靠各自 margin 凑）；**位置 / 显示列 / PlayStyle 三个多选下拉**（原生 Popover 面板，进 top layer 不被抽屉裁切，触发器贴视口底时自动翻到上方），位置只放 12 个码位、PlayStyle 顶层分「银徽章 / 金徽章」两段（各 36 项，段内按射门/传球/防守/控球/体格/门将分组）、显示列的「恢复自动」移入面板页脚；「更多筛选」里**同一属性的上下限成对一行两列**（左最低、右最高，7 对区间 + 属性 1 对 + 合同 3 对）、属性下拉按七组小标题写中文名；生效条件摘要条**与翻页条合并成表格正上方一行**（chips 靠左、上一页/共 N 名球员/下一页靠右），无筛选时不渲染摘要条，位置一条 chip 清整类、银/金徽章各一条；**排序改点表头**（29 键、两态循环升→降→升、默认态不给任何列打 active）+ keyset 游标分页（游标栈回翻）；姓名搜索**去变音**（「sesko」能搜到「Šeško」），聚焦才预载轻量名册做本地推荐、点条目跳详情；家族表格列 UID/姓名(链接)/归属/位置/年龄/CA/PA/成长胶囊/声望/身价/状态徽章/徽章数；不显示工资——那是合同卷宗里的事（增量 26 改版、增量 27 收口） |
