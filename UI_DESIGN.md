...
| 球员详情 | `.dossier` 双栏：左球员卡常驻（可成长/非成长胶囊），右侧页内页签 `.seg` 合同/属性/成长——合同=合同卷宗；属性=FC 源数据重排（位置矩阵主位深色、角色带 ++金/+蓝、星级行 ★☆、六组细分卡绿橙红三档色阶、门将追加 GKP 共七组、六维雷达静态 SVG 门将换轴 DIV/HAN/KIC/REF/POS/SPD、PlayStyles 徽章——**按槽位渲染全 15 槽**（银 `PSID1-12` + 金 `PSID13-15`，只渲染有值的槽、不给空槽占位，增量 29）；成长=卷宗时间线（增量 14 删掉「初始归属」行） |
