...
**状态**：**已上线（2026-09-26，用户指令「推送」）**——迁移 `0042` 已于 push 前 apply 到生产（`Executed 2 commands in 47.58ms` ✅；只读核验索引 SQL 与本地逐字一致）；push `e95122c..a8bb833`（5 个提交 = v6.4.1 的 `f768a01` + 文案 `18f37c8` + v6.5.0 三枚）约 1 分钟后 Workers Builds 自动部署 Version **`3bea29d7-4cca-403e-b0b1-34ddeae0154a`**（12:45:07Z）；上线回读 health / `?marker=ge90` / `sort=marker` / 组合筛选排序 / clubs / squads 全 200，生产列表实测出 `"marker":"ge90"`；线上首页资产 `index-BdMUWht2.js` 与本地 dist 逐字一致（首次比对不一致是因为本地 dist 构建于 bump 版本号之前——`__APP_VERSION__` 注入不同，重构建后一致）。判级 minor：新增用户可见能力（标记列/筛选/排序/球员卡徽标）+ 迁移 `0042`。提交 `0221e29` + `f0e959f`，前者后走过 code-review。
...
- **还剩 6 个**（`growth_gap` / `growth_tier` / `future_star` / `china_plan` / `agent_tier` / `fc_id`），仍按每天最多 3 条分批；`view=initial` 口径另欠 `pa` / `growth_gap` 两个变体。**代价**：`players` 索引 16 → **21** 条，全量重导 upsert 成本随之从 201,311 涨到 **256,214** 行（口径见 `scripts/players-import/README.md`）。证据与逐条豁免理由见 `scripts/d1-read-audit/README.md` §3.3 / §5.1 / §5.3 / §5.5。⇒ **此行的「还剩 6 个 / 索引 21 条」是 batch 4/5 收口时的账面；`growth_tier` 与 `future_star` 两条已由下一节 batch 6 收掉（剩 4 个、索引 23 条）**。

**第 5 节续做 · 排序索引 batch 6（2026-09-25 已 apply 到生产）**：本批**换了选键依据** —— batch 4/5 是「配额能推几条推几条」，本批先查清「这 6 个键到底有没有人用」再选，查证结果推翻了原计划。
...
- **设计裁决（否掉「排序改裸列 + 普通列索引」）**：排序侧不动（仍 `COALESCE(col, 0)`），索引建 `(COALESCE(col, 0), id)`，**筛选侧改成 `COALESCE(col, 0) = ?` 与索引同源**。理由：keyset 游标拿排序表达式当键，裸列一旦为 NULL 比较恒为假会**静默漏行**（`src/worker/routes/players.ts:96` 的 years 注释写明这条规矩），而这五列在 `src/db/migrations/0001_init.sql:18-23` 是可空的（生产当前 NULL 数 0，但口径不该依赖数据现状）。两条路的写配额相同。
- `src/db/migrations/0038_players_sort_indexes_batch6.sql`（**2026-09-25 已 apply**，报 `Executed 3 commands in 62.85ms`）：`idx_players_sort_growth_tier`（`COALESCE(growth_tier, 0), id`）、`idx_players_sort_future_star`（`COALESCE(is_future_star, 0), id`）。**apply 绕开未授权的 `0037`**：`0037_offers.sql` 是另一会话在途的报价子系统迁移，用仓库配置跑 `d1 migrations apply` 会把它一起 apply ⇒ 用临时配置 `scratch/wrangler-0038.jsonc`（`migrations_dir: "migrate-0038"`，目录里只放 0038）先 `migrations list --remote` 确认只剩 0038 再 apply；apply 后账本核对最新 = `0038`、上一条 = `0036` ⇒ 0037 未被 apply。
- 同源锁：`scripts/measure-d1-reads.mjs` 补 **9 条探针**（6 个未建索引键 + `view=initial&sort=pa` + 2 条筛选形状）；`tests/players-sort-indexes.test.ts` 元组加第 4 元素 `filter`、新增 2 条「等值筛选走同一条索引」用例、`INDEXED_SORTS` 16 → **18 条**；`tests/d1.ts` 的 `MIGRATION_FILES` 追加 `0038`。
- **关键发现（SQLite 计划器，`scratch/probe-0038-plan.mjs` 七种组合实测）**：索引首列被等值约束时，SQLite **不再用它出 ORDER BY** —— 只排序 → `SCAN … USING COVERING INDEX`（有序、无临时排序）；筛选 + 同键排序（含 ASC / 无 id 尾列变体）→ `SEARCH … USING COVERING INDEX (…=?)` + **`USE TEMP B-TREE FOR ORDER BY`**。⇒ 筛选侧收益是「读量从全表扫降到命中子集」，不是提前停；用例因此只锁 `SEARCH`，不断言无临时排序。
- **收益实测**：`sort=growth_tier` 37,635 → **22**、`sort=future_star` → **22**、`sort=growth_tier&growth_tier=3` 18,302 → **1**、`sort=future_star&is_future_star=1` 18,504 → **307**（读数落 `scripts/d1-read-audit/measurements-after.json`）；未建索引的 `china_plan` / `agent_tier` / `fc_id` / `growth_gap` / `view=initial&sort=pa` 仍 37,635（把 §5.1 的类级推断再实测一次）。生产 `sqlite_master` 核对 `idx_players_sort_%` **16 → 18 条**；生产 EXPLAIN：排序 `SCAN players USING COVERING INDEX idx_players_sort_growth_tier`、筛选 `SEARCH players USING COVERING INDEX idx_players_sort_growth_tier (<expr>=?)`。
...
- **还剩 4 个**（`china_plan` / `agent_tier` / `fc_id` / `growth_gap`）+ `view=initial` 口径的 `pa` / `growth_gap` 两个变体；这四个键与 `growth_tier` 同性质（排序表达式 + 裸列筛选），可照本批配方用一条索引收两面。**代价**：`players` 索引 21 → **23** 条（upsert 重导成本口径需按同一方法重算，本批未做）。证据与逐条豁免理由见 `scripts/d1-read-audit/README.md` §5.1 / §5.5。
