...
**状态**：a/b/c/d 代码+测试完成（284 测试绿 + tsc ✓ + build ✓），待 code review 与提交。

## 增量 11 · 赛季结算域（立项）

奖金自动发放（赛果确认即时入账可算部分+小组赛阶段结算按胜场比例分总池）、赛季结算按钮（手动触发+前置校验）、忠诚奖金、growable 重判（规则 4.1.1：大版本第 1/2/3+ 季可成长年龄上限 25/24/23 递减，growable=CA<PA 且 age≤本季上限）、下季封存初始化。依赖增量 10 完成。

## 增量 12 · 主场收入域（立项）

主场收入公式（revenue 插件同构移植）、设施经营（升级/维护费，设施数据模型一次建全）、死忠演化。依赖增量 11 完成。
