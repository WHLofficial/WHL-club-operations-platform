...
| 球员库 D1 读消耗的量化与治理（增量 27 步骤 1 原计划：生产 `rows_read` 抽样 + 按 URL 统计重复率与命中率 + 必要时只读统计端点；再据此选「缓存/查询微调 / 静态快照 / 只改前端」） | 增量 28 | **等令**——用户 2026-09-21 裁决整条挪到增量 28（当日生产 D1 读额度已超限，免费档按 UTC 零点归零；量化本身要先打生产，故与原计划自相矛盾）。**2026-09-21 已从「优化项」升级为当日中断事故**：15:20Z 起 worker 内所有真实表读失败（`/api/players*` 一律 500、cron 每 5 分钟失败一次），根因确认为免费档 5,000,000 行/日读配额耗尽（当日 whl-club 已读 4,350,235 行 / 5311 次读查询）；限额按 UTC 零点归零。增量 28 的量化要连带回答「每页两条查询（主查询 + 无 cursor 的全表 COUNT）的读放大怎么砍」。背景与量级估算见 TECH_DESIGN §17 与本节各增量的缓存/分页口径 |
