...
- **增量 28（球员库 D1 读消耗量化与治理）进行中**：步骤 0–4 已完成并提交（`b7ca169` 量化报告 = 30 形状 + 6 探针实测；`d9beff0` 去整表 COUNT + `total` 改游标式 + 内部计数端点 `GET /api/cron/players-count`，fail-closed + 只认 `X-Cron-Key` 头；`4fb483a` 生产补配 CRON_KEY 的文档；`3671c33` 分级 TTL + 边缘 Cache API + 代际键 purge + 中心化挂钩；步骤 4 迁移 `0029` 三条表达式索引 + 同源锁死测试），**未 push、未部署**。已落地的读量改进：默认浏览一页 18,819 → 56 行（命中缓存 0 行）；`sort=prestige/club/status` 56,398 → 53/43/22 行（迁移 `0029` 已于 2026-09-22 apply 到生产）。剩余：前端 `staleTime`、全站读面普查、验收复测与部署。
