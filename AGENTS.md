...
- 生产最新 Version **`91635aca-92d3-4921-8208-d9f7912aaec7`**（2026-09-22T04:19:18Z，含增量 27 + 增量 28 步骤 0–5；`wrangler deploy` CLI 回显的 Current Version ID 为 `4d6119f4-aa22-47ea-979b-ce9a46c6736f`）；上一版为 `31c58da7-6611-4634-8c22-678a0f9e6876`（2026-09-22T02:17:47Z，Source `Secret Change`——配 `CRON_KEY` 触发的换版，代码与 `3455087e` 相同），再上一版 `3455087e-4bdb-4a63-b40f-8df943d80892`（2026-09-21T17:13:35Z，含增量 27），再上一版 `b83ec876-9fc7-4c16-8d01-0cba3d8ab5e5`（2026-09-21T14:39Z，含增量 25–26）。`wrangler deploy` CLI 回显的 Current Version ID 与 `wrangler deployments status` 里的 Version 是两个值（增量 26 为 `81e93c74-…` vs `b83ec876-…`，增量 27 为 `ff9c2e91-…` vs `3455087e-…`，增量 28 为 `4d6119f4-…` vs `91635aca-…`）。**配 secret 也会换版本**（`wrangler secret put` 生成 Source `Secret Change` 的新版本）。生产 vars 自增量 28 起**不再配 `PUBLIC_CACHE_TTL_MS`**（缓存口径改由 `src/lib/cache-policy.ts` 的分级表承载，变量退化为显式覆盖）。生产迁移已到 **0029**（`0029_players_sort_indexes_batch2.sql` 于 2026-09-22 单独 apply，增量 28 步骤 4；`d1 migrations list whl-club --remote` 报 No migrations to apply）。**迁移一旦 apply 到生产就不得再改**，要改新增下一个文件。网页面板建合同不再落 DDL 默认刻度（增量 25 代码已上线）。
...
- **增量 28（球员库 D1 读消耗量化与治理）进行中，步骤 0–5 已于 2026-09-22 推送（`994ded9..dbf55c7`，6 个提交）并部署上线（生产 Version `91635aca-…`）**：步骤 0–1 量化（`b7ca169`，30 形状 + 6 探针实测报告落 `scripts/d1-read-audit/`）；步骤 2 去整表 COUNT + `total` 改游标式 + 内部计数端点 `GET /api/cron/players-count`（`d9beff0`，fail-closed + 只认 `X-Cron-Key` 头；`4fb483a` 为生产补配 CRON_KEY 的文档）；步骤 3 分级 TTL + 边缘 Cache API + 代际键 purge + 中心化挂钩（`3671c33`）；步骤 4 迁移 `0029` 三条表达式索引 + 同源锁死测试（`c6d1d22`，已 apply 到生产）；步骤 5 前端 `staleTime`（`dbf55c7`，全局 30s / 列表 60s，并补两处写后失效）。**已落地的读量改进**：默认浏览一页 18,819 → 56 行（命中缓存 0 行）；`sort=prestige/club/status` 56,398 → 53/43/22 行。**剩余**：全站读面普查（步骤 6）、验收复测与最终回读（步骤 7）、文档与记忆收口（步骤 8）。部署后回读已验证：`/api/health` 三资源 ok、列表响应**已无 `total`** 且有 `nextCursor`、`sort=name`/`sort=prestige`/`ps=125` 全 200、计数端点无密钥 403 / 带密钥 `count=18301`、名册 200，线上 HTML 引用 `index-DZu3s6Fn.js` 与本地产物一致。
