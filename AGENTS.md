...
- **增量 25 与增量 26 均已本地收口、未 push 未部署**（截至 2026-09-21 领先 `origin/main` 39 个提交：增量 25 一行 21 个 + 增量 26 的 18 个（16 个功能与测试 + 2 个收口文档）；ROADMAP 增量 25 节的「交付」只列 5 个代码提交，另 16 个是随后的 S9 生产批工件与执行记录）。增量 26 是球员库筛选搬进左栏（左栏 / 表头点排序 / 姓名去变音搜索 / 窄屏抽屉），只改代码与文档、**未写任何生产数据**（折叠表依据生产库只读扫描 `scripts/scan-name-chars.mjs --remote`，未写）。
- 生产最新 Version `90bfd78f-fae4-4584-8d52-871486aa46c0`（2026-09-20T05:42Z，含增量 17–24）；生产迁移已到 **0028**（`0028_contract_window_ticks.sql` 于 2026-09-21 随合同导入批 apply）。**增量 25 的 worker 代码未部署**（`npm run deploy` 未跑；网页面板建合同会落 DDL 默认刻度，下次开窗前应先部署）。
