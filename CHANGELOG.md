...
## [未发布] · 增量 17–24（2026-09-19 ~ 2026-09-20 本地完成，未 push 未部署）

生产最新部署仍是增量 16，生产迁移停在 0021，0022–0027 待 apply。
...
- 删除过期草稿 `handoff-20260916.md`（停在增量 12），其中仍有效的三条（18408 导入源数据位置与口径、导入通道选择、`--file` 通道外键陷阱）已迁入 ROADMAP 与 `scripts/README.md`（增量 24）。

### 待办

- push 与部署等令；部署日注意：0027 会一次性写约 7.3 万行（4 条索引 × 18301 名球员），D1 免费档日写配额 10 万行，需单独安排。
- 生产回填待执行：16 队队籍（`scripts/prod-20260919-roster-backfill/`，444 人，只写队籍不造合同）。
- 部署后必查：`clubs.is_cpu` 四个 CPU 队（id 10 / 241 / 112172 / 131681）回填结果。
